...
case "$1" in
  "setup")
    (
      set -x
      sidecar_version=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/mutagenio/sidecar/tags?page_size=1000' | jq --raw-output '.results[].name' | sort -r | grep -v 'beta' | grep -v 'alpha' | grep -v 'sspl' | head -n 1)
